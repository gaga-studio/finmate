...
새 회원가입 사용자는 빈 상태로 시작합니다. QA나 데이터 연동 검증이 필요할 때는 `tools/scripts/import-synthetic-mydata.py`로 `gaga-studio/financial-sns-mydata-202606` 데이터셋을 Postgres에 import해 `p001@synthetic.finmate.local` 같은 synthetic 계정을 사용합니다.
