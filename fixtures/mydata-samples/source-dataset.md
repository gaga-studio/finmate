...
원본 데이터는 데모 설계의 출처이며, FinMate seed는 발표 흐름에 맞게 정규화한다. Source persona는 페르소나 성격과 거래 맥락의 출처이고, seed JSON은 P0 화면과 계산 흐름을 안정적으로 보여주기 위한 정규화 실행 데이터다.

| 항목 | 정책 |
| --- | --- |
| 화면 수치 | 발표 중 계산이 쉽게 보이도록 seed에서 고정 |
| 시뮬레이션 | `400000 + 100000 * 3 = 700000` 흐름을 우선 |
| 유사도 점수 | `similarityScore=0.84`는 `DEMO_NORMALIZED` 점수이며 원본 feature matrix의 raw cluster distance가 아님 |
| 정규화 맵 | `fixtures/mydata-samples/p0/normalization-map.json`에서 source feature와 FinMate seed 값을 나란히 확인 |
