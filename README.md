...
현재 레포는 P0 데모 구현 전 계약/문서/seed/검증 패키지입니다. 실제 app code는 이후 `apps/web`, `apps/api`, `packages/shared` 또는 별도 구현 단계에서 추가합니다.

향후 이 레포에 구현을 붙인다면 권장 구조는 `apps/web`(프론트), `apps/api`(백엔드/API), `packages/shared`(공통 DTO/schema)입니다. 이번 v1.0 패키지에는 실제 앱 코드를 포함하지 않습니다.
...
같은 검증은 GitHub Actions의 `Contract Check` workflow에서도 `push`와 `pull_request`마다 실행됩니다.
