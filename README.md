...
현재 레포는 P0 데모용 계약/문서/seed/검증 패키지, Spring Boot mock API, Vite React 기반 web skeleton을 포함합니다. 실제 DB, 실제 마이데이터, LLM provider는 아직 포함하지 않습니다.
...
API 서버를 먼저 실행한 뒤 별도 터미널에서 Vite web skeleton을 실행합니다.
...
브라우저에서 `http://localhost:5173/onboarding`을 열고 다음 클릭 흐름을 확인합니다.
...
web skeleton은 `VITE_API_BASE_URL`이 없으면 `http://localhost:8080`을 사용합니다.
...
| 프론트엔드 | `apps/web/src`, `docs/02_screen_state_spec_v1.0.md`, `openapi/finmate-p0-v1.0.yaml`, `requests/finmate-p0-v1.0.http` |
...
검증 스크립트는 OpenAPI 12개 API, seed 고정 ID, 시뮬레이션 수치, 개인정보 철회 대상, 화면에서 필요한 응답 필드, 데이터셋 subset 경계, 금지된 버전/표현 회귀를 확인합니다.
...
3. `requests/finmate-p0-v1.0.http`의 12단계 P0 흐름과 privacy verification 2개를 순서대로 확인합니다.
