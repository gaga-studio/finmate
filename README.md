...
현재 레포는 P0 데모용 계약/문서/seed/검증 패키지와 Spring Boot mock API를 포함합니다. 실제 DB, 실제 마이데이터, LLM provider, 프론트 앱은 아직 포함하지 않습니다.

향후 프론트와 공통 타입을 붙인다면 권장 구조는 `apps/web`(프론트), `apps/api`(백엔드/API), `packages/shared`(공통 DTO/schema)입니다.
